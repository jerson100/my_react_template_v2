...
import { IsCoverFaceAtom, SpreadFingersAtom } from "../states/login-atoms";
...
import { G } from "@/core/components/chakraui/svg/g";
import { Polygon } from "@/core/components/chakraui/svg/polygon";
...
const Face = () => {
  ...
  const startPath =
    "M200,122h-35h-14.9V72c0-27.6-22.4-50-50-50s-50,22.4-50,50v50H35.8H0l0,91h200L200,122z";
  const endPath =
  ...
  const isCoverFace = useAtomValue(IsCoverFaceAtom);
  const spreadFingers = useAtomValue(SpreadFingersAtom);
  ...
  useEffect(() => {
    const _interpolar = (initial: string, end: string) => {
      const interpolator = interpolate(initial, end);
      let progress = 0;

      const animatePath = () => {
        if (progress <= 1) {
          // Obtener el path interpolado
          const interpolatedPath = interpolator(progress);
          setCurrentPath(interpolatedPath); // Actualizar el path

          // Control de animación con Framer Motion
          controls.start({
            path: interpolatedPath,
            transition: { duration: 0.45, ease: "easeInOut" },
          });

          progress += 0.04; // Incrementar el progreso
          requestAnimationFrame(animatePath); // Continuar la animación
        }
      };
    ...
    if (isCoverFace) {
      _interpolar(startPath, endPath);
    } else {
      _interpolar(endPath, startPath);
    }
        ...
        >
          ...
          <g className="body">
            ...
            <MotionPath
              ...
              animate={controls}
              d={currentPath}
              variants={CoverFaceVariants}
            />
            <Path
              fill={{
                _dark: `${selectedPalette}.600/50`,
                _light: `${selectedPalette}.500/20`,
              }}
          ...
          <g className="eyeL">
            <Circle
              ...
              cy="78.5"
              r="3.5"
              fill={{ _dark: bgHover, _light: `${selectedPalette}.600/50` }}
            />
          ...
          <g className="arms" clipPath="url(#armMask)">
            ...
            <motion.g
              style={{ transformOrigin: "top left", x: "-93px", y: "220px" }}
              className="armL"
              initial="initialARML"
              animate={isCoverFace ? "animateARML" : "initialARML"}
              variants={CoverFaceVariants}
            >
              <Polygon
                fill={{ _dark: bgHover, _light: "#fff" }}
              ...
              <motion.g
                ...
                initial="hidden"
                animate={spreadFingers ? "animate" : "hidden"}
                variants={TwoFingersVariants}
              >
                <Path
                  fill={{ _dark: bgHover, _light: "#fff" }}
                  stroke={`${selectedPalette}.600/50`}
            ...
            <motion.g
              style={{ transformOrigin: "top right", x: "-93px", y: "220px" }}
              className="armR"
              initial="initialARMR"
              animate={isCoverFace ? "animateARMR" : "initialARMR"}
              variants={CoverFaceVariants}
            >
              <Path
                fill={{ _dark: bgHover, _light: "#fff" }}
